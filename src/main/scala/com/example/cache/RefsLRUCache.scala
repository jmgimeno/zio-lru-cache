...
final class RefsLRUCache[K, V] private (
...
object RefsLRUCache {
  def make[K, V](capacity: Int): IO[IllegalArgumentException, RefsLRUCache[K, V]] =
    if (capacity > 0) {
      for {
        itemsRef <- Ref.make(Map.empty[K, CacheItem[K, V]])
        startRef <- Ref.make(Option.empty[K])
        endRef   <- Ref.make(Option.empty[K])
      } yield new RefsLRUCache[K, V](capacity, itemsRef, startRef, endRef)
    } else {
